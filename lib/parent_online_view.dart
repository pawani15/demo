...
import 'Student/Subject_AnswerDialog.dart';

class ParentOnlineView extends StatefulWidget {
  final String studentname,studentid;
  ParentOnlineView({this.studentname,this.studentid});
  @override
  State<StatefulWidget> createState() => new _ParentOnlineViewPageState();
}

class _ParentOnlineViewPageState extends State<ParentOnlineView> with SingleTickerProviderStateMixin
{
  ...
  Future<dynamic>getAnswersData(String id1)async{
    ...
    var url = await Constants().Clienturl() + Constants.parent_view_result +
        widget.studentid;
    Map<String, String> body = new Map();
    body['login_user_id'] =   widget.studentid;
                                  ...
                                  children: <Widget>[
                                    ...
                                    Row(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Text(
                                          'Section:'+ resultData['section_name'],
                                          style: TextStyle(color: Colors.white),
                                        ),
                                      ],
                                    ),
                                    ...
                                    Text(
                                      'TotalMarks'+resultData['total_marks'].toString(),
                                      style: TextStyle(color: Colors.white),
                                    ),
                                    Text(
                                      'Time'+resultData['duration_time'],
                                      style: TextStyle(color: Colors.white),
                                    ),
                                    GestureDetector(
                                      child: Text(
                                      ...
                                      onTap: () {
                                        _showDialog(
                                            context, resultData['instruction']);
                                      },
                                    )
                                  ],
                                ),
  ...
  Future<Null> Loadonlineviewanswer() async {
    ...
    var url = await Constants().Clienturl() +
        Constants.parent_view_online_exam +   widget.studentid;;
    Map<String, String> body = new Map();
                    ...
                    _Onlineexamdetails.length == 0 ? new Container(
                      ...
                      child: DataTable(
                        ...
                        columns: [
                          ...
                          DataColumn(
                            label: Text("Obtained Marks",style:  TextStyle(  color: Color(0xff182C61), fontWeight: FontWeight.bold,fontSize: 15.0),),
                          ),
                          DataColumn(
...
class Onlineexamdetails {
  String examname, id, subname, examdate, time, timeStart, timeEnd, totalmarks,sno,
      obtmarks, result, onlineexamid, examdate1;

  Onlineexamdetails(
      {this.id, this.examname, this.subname, this.examdate,this.sno ,this.timeStart, this.timeEnd, this.time, this.totalmarks, this.obtmarks, this.result, this.onlineexamid, this.examdate1});
