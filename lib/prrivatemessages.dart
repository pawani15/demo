...
class PrivatemessagesPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new _PrivatemessagesPageState();
}

class _PrivatemessagesPageState extends State<PrivatemessagesPage> with SingleTickerProviderStateMixin{
...
class _NewMessagesState extends State<NewMessages>{
  ...
  Future<Null> LoadPrivatemessages() async {
    ...
    var url = await Constants().Clienturl() + Constants.Load_Privateusers +id;
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          try {
            for (Map user in responseJson['result']) {
              _MessageDetails.add(MessageDetails.fromJson(user));
            }
          }catch(e){
                  ...
                  new InkWell(child:new Row(
                    ...
                    onTap: (){
                      Navigator.of(context).push(new MaterialPageRoute(
                          ...
                          new PrivatechatPage(messageDetails: _MessageDetails[index],)));
                    },
                  ),
                  new SizedBox(width: 5,height: 5,),
  ...
  List<MessageDetails> _searchResult = [];
...
class MessageDetails {
  ...
  MessageDetails({this.name, this.message, this.photo,this.date,this.messagethread});
...
class _GroupMessagesState extends State<GroupMessages> {
  ...
  Future<Null> LoadGroupmessages() async {
    String id = await sp.ReadString("Userid");
    var url = await Constants().Clienturl() +Constants.Load_Groupusers +id;
    Map<String, String> body = new Map();
    body['teacher_id'] = id;
    print("url is $url"+"body--"+body.toString());
    http.get(url,
                  ...
                  children: <Widget>[
                    new InkWell(child: new Row(
                      ...
                      onTap: () {
                        Navigator.of(context).push(new MaterialPageRoute(
                            builder: (BuildContext context) =>
                            new GroupchatPage(messageDetails: Groupmessageslist[index],)));
                      },
                    ),
                    new SizedBox(width: 5, height: 5,),
                    new Divider(height: 1, color: Colors.grey,),
