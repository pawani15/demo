...
class ManageStudentProfilePage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new _ManageStudentProfilePageState();
}

class _ManageStudentProfilePageState extends State<ManageStudentProfilePage> {
  bool notification = false,locationtracking = false;
  String student= "",email='';
  bool _loading = false;
  TextEditingController studcont= new TextEditingController();
    ...
    http.post(url,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          try {
            studcont.text = responseJson['result'][0]['name'];
  ...
  Future<Null> Updateemail() async {
    ...
    if(Constants.logintype == "teacher") {
      url = await Constants().Clienturl() + Constants.Manage_Profile +
          await Constants().Userid();
    }

    body['parent_id'] = id;
    body['email'] = emailcont.text;
    body['name'] = studcont.text;
                    ...
                    children: <Widget>[
                      new Expanded(child: new Container(
                          ...
                          child: new InkWell(
                              onTap: () {
                                setState(() {
                                  currentpassword.text='';newpassword.text='';confirmnewpassword.text='';studcont.text="";emailcont.text="";
                                });
                              },
                              child: new Container(
