...
class StudentSubjectWiseAttendance extends StatefulWidget {
  int currentIndex = 0;
  num present,
      absent,
      sickleave,
  ...
  @override
  State<StatefulWidget> createState() => new _StudentSubjectWiseAttendancePageState();
}

class _StudentSubjectWiseAttendancePageState extends State<StudentSubjectWiseAttendance> {
  //num halfLeave;
  @override
  void initState() {
    super.initState();
...
//    body['class_name'] = id;
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          for (Map data in responseJson['result']['subjects']) {
           widget.subjectlist.add(data['subject_name']);
            widget.subjectmap[data['subject_name']] = data['subject_id'];
          }
        }
        setState(() {
          widget._loading = false;
  ...
  _navigatetosubjects(BuildContext context) async {
    String result = await Constants().Selectiondialog(context, "subjects", widget.subjectlist);
    setState(() {
      widget.subject = result ?? widget.subject;
    });
  ...
  Future<Null> GetRunningyear() async {
    if (widget.fromdate != null && widget.todate != null) {
      String id = await sp.ReadString("Userid");
      // var url =await Constants().Clienturl() + "api_students/attendance_report_new/";
      var url = await Constants().Clienturl() +
          Constants.Student_SubjectWiseAttendance;
      Map<String, String> body = new Map();
      body['student_id'] = id;
      body['from_date'] = widget.fromdate;
      body['to_date'] = widget.todate;
      body['subject_id']=widget.subjectmap[widget.subject];
      print("print+++:::::"+widget.subjectmap[widget.subject]);
      http.post(url,
...
//            print ("dfasd"+  month2.length.toString());
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
        appBar: new AppBar(
          title: Container(child: Text("Student SubjectWise Attendence",style: TextStyle(fontSize: 13), )),
                          ...
                          onTap: () {
                            setState(() {
                              ...
                              if (widget.currentIndex != 0) {
                                widget.varList.clear();
                                widget.varList.addAll(
                                    widget.month2[widget.currentIndex]
                                    ["attendance_list"]);
                        ...
                        child: Wrap(
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              ...
                              style:
                              TextStyle(color: Colors.yellowAccent[800]),
                            ),
                                      ...
                                      //height: 300,
                                        child: Container(
                                          ...
                                          child: new ListView.builder(
                                            ...
                                            itemCount: widget.sortedMonthDate
                                                .length,
...
//
  ...
  setFinals(List varList) {
    ...
    for (Map<String,dynamic> obj in varList) {
      ...
      else if (obj.containsKey('T/P'))
        widget.totalPresent = obj['T/P'];
      else if (obj.containsKey('OL'))
        widget.otherleave = obj['OL'];
      else if (obj.containsKey('SL'))
        widget.sickleave = obj['SL'];
      else if (obj.containsKey('T/U'))
        widget.totalUndefined = obj['T/U'];
      else if (obj.containsKey('CL'))
        widget.casualleave = obj['CL'];
      else if (obj.containsKey('HL'))
        widget.halfLeave = obj['HL'];
      else if (obj.containsKey('A'))
        widget.absent = obj['A'];
      else if (obj.containsKey('E'))
        widget.event = obj['E'];
      else if (obj.containsKey('P'))
        widget.present = obj['P'];
    ...
    widget.sortedMonthDate = widget.selectedMonth.keys.toList()
      ..sort();
    setState(() {});
  }}
