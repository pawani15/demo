...
class StudentPayment extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => new _StudentPaymentePageState();
}
...
class _StudentPaymentePageState extends State<StudentPayment>
{
  ...
  Future<Null> LoadFee() async {
    ...
    var url = await Constants().Clienturl() + Constants.Student_Fee_group+id;
    ...
    http.post(url,
      ...
      if (response.statusCode == 200) {
        ...
        if (resJsonOne['status'].toString() == "true") {
          ...
          print(id1);
          var url2 = await Constants().Clienturl() + Constants.Student_Fee_+id1;
          Map<String, String> body2 = new Map();
          body2['fee_clctn_id'] = id1;
          print("url is $url2"+"body--"+body2.toString());
          http.post(url2,
              headers: {"Content-Type": "application/x-www-form-urlencoded"},
              body: body2 ).then((response)async{
            if (response.statusCode == 200) {
...
//                                  ),
                                  DataColumn(
                                    label: Text("Group Name",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  DataColumn(
                                    label: Text("Fees Name",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  DataColumn(
                                  ...
                                  DataColumn(
                                    label: Text("Mode",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  ...
                                  DataColumn(
                                    label: Text("Paid",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  DataColumn(
                                    label: Text("Discount",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  DataColumn(
                                    label: Text("Fine",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
                                  DataColumn(
                                    label: Text("Balance",style:  TextStyle(color: Theme.of(context).primaryColor, fontSize: 15.0),),
                                  ),
...
//                                        ),
                                        DataCell(
                                          user.grpName==null?Text(" "):  Text(user.grpName),
                                        ),
                                        DataCell(
                                          user.feeName == null
                                              ? Text(" ")
                                        ...
                                        DataCell(
                                          user.dueDate==null?Text(" "):  Text(user.dueDate.toString()),
                                        ),
                                        DataCell(
                                          user.status==null?Text(" "):  Text(user.status),
                                        ),
                                        DataCell(
                                          user.totalAmount==null?Text(" "):   Text(user.totalAmount),
                                        ),
                                        DataCell(
                                          user.paymentsInfoId==null?Text(" "):  Text(user.paymentsInfoId),
                                        ),
                                        DataCell(
                                          user.paymentMode==null?Text(" "):  Text(user.paymentMode),
                                        ),
                                        DataCell(
                                        ...
                                        DataCell(
                                          user.totalPaid==null?Text(" "):  Text(user.totalPaid),
                                        ),
                                        DataCell(
                                          user.discountAmount == null ? Text(
                                              " ") : Text(
                                              user.discountAmount.toString()),
                                        ),
                                        DataCell(
                                          user.fineAmount==null?Text(" "):   Text(user.fineAmount),
                                        ),
                                        DataCell(
                                          user.dueAmount==null?Text(" "):   Text(user.dueAmount),
                                        ),
...
//  }
  ...
  onSearchTextChanged(String text) async {
    ...
    _Homeworkdetails.forEach((vehicleDetail) {
      if (vehicleDetail.grpName.toLowerCase().contains(text.toLowerCase()) || vehicleDetail.fineAmount.toLowerCase().contains(text.toLowerCase())
          || vehicleDetail.dueDate.toString().toLowerCase().contains(text.toLowerCase()))
...
class Paymentdetails {
  String feesgrp,status,mode,discount,feestype,amt,date,fine,DueDate,paymentid,paid,bal,sno;
  Paymentdetails({this.feesgrp, this.status, this.mode, this.discount,this.feestype,this.amt,this.date,this.fine,this.DueDate,this.paymentid,this.paid,this.bal,this.sno});
...
//      id: json[''].toString() ,
      ...
      date: new DateFormat('dd-MM-yyyy').format(
          new DateTime.fromMillisecondsSinceEpoch(
              int.tryParse(json['date']) * 1000)).toString() == null
          ? null
          : json['date'].toString(),
      fine:json['fine_amount'].toString(),
      DueDate: new DateFormat('dd-MM-yyyy').format(
          new DateTime.fromMillisecondsSinceEpoch(
              int.tryParse(json['due_date']) * 1000)).toString(),
      paymentid: json['payments_info_id'].toString() ==  null? null:json['payments_info_id'].toString(),
...
class Payments {
  ...
  factory Payments.fromJson(Map<String, dynamic> json) => Payments(
    ...
    detailedInvoiceId: json["detailed_invoice_id"] == null ? null : json["detailed_invoice_id"],
    paymentsInfoId: json["payments_info_id"] == null ? null : json["payments_info_id"],
  ...
  Map<String, dynamic> toJson() => {
    ...
    "detailed_invoice_id": detailedInvoiceId == null ? null : detailedInvoiceId,
    "payments_info_id": paymentsInfoId == null ? null : paymentsInfoId,
    "payment_date": paymentDate == null ? null : paymentDate.toString(),
