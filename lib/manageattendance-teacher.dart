...
class ManageTeacherattendancePage extends StatefulWidget {
  final String id;
  ManageTeacherattendancePage({this.id});
  @override
  State<StatefulWidget> createState() => new _ManageTeacherattendancePageState();
}
...
class _ManageTeacherattendancePageState extends State<ManageTeacherattendancePage> {
  ...
  String exam = "",classname='',sectionnamme='',status='Select',section = '',classId='',classid='',sectionid='',timestamp='';
  bool _loading = false;
...
//  }
  ...
  LoadCLassdetails() async{
    ...
    Map body = new Map();
    body['teacher_id'] = await Constants().Userid();
    var url = await Constants().Clienturl() + Constants.Load_Classes+widget.id;
    print("url--"+url+'body is${json.encode(body)} $body');
    http.post(url,headers: {"Content-Type": "application/x-www-form-urlencoded"},body:body)
        .then((response) {
      if (response.statusCode == 200) {
  ...
  Updateattendance() async{
    ...
    var url = await Constants().Clienturl() + Constants.Update_Attendance+classid;
    print("url--"+url+'body is${json.encode(body)} $body');
  ...
  LoadClass_Sectiondetails() async{
    ...
    body['teacher_id'] = await Constants().Userid();
    body['class_id']=widget.id;
    var url = await Constants().Clienturl() + Constants.Load_Sections_Dropdown+widget.id;
    print("url--"+url+'body is${json.encode(body)} $body');
    http.post(url,headers: {"Content-Type": "application/x-www-form-urlencoded"},body:body)
        .then((response) {
      if (response.statusCode == 200) {
  ...
  Future<Null> ShowAttendance() async {
    ...
    Constants().onLoading(context);
    var url = await Constants().Clienturl() + Constants.Manage_Attendance_Teacher+widget.id;
    ...
    http.post(url,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson != null) {
          print("response json ${responseJson}");
          setState(() {
            if(responseJson['result'].containsKey("attendance_of_students")) {
              Attendancelist = responseJson['result']['attendance_of_students'];
  ...
  _navigatetoclass(BuildContext context) async {
    String result = await Constants().Selectiondialog(context, "class", classlist);
    setState(() {
      section = result ?? section;
    });
    print("res--"+result.toString());
