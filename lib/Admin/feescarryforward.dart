...
class feesCarryforwardPage extends StatefulWidget {

  @override
  State<StatefulWidget> createState() => new _feesCarryforwardPageState();
}

class _feesCarryforwardPageState extends State<feesCarryforwardPage> {
  String exam = "",
      clas = '',
      section = '',
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          try {
              for (Map user in responseJson['result']['std_all']) {
                _Feescarryforwarddetails.add(feescarryforwarddetails.fromJson(user));
              }
          }catch(e){
            _Feescarryforwarddetails = new List();
          }
        }
        setState(() {
          _loading = false;
        ...
        children: <Widget>[
          new Container(
            ...
            child: new Container(
              ...
              child: new Text("Fees Carry Forward",style: TextStyle(color: Colors.white,fontSize: 16.0,fontWeight: FontWeight.bold),textAlign: TextAlign.center,),
            ),
          ),
          new Container(
            ...
            child: new ListView(
              children: <Widget>[
                ...
                new Card(
                  ...
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(30)),side: BorderSide(width: 1,color: Colors.red[700])),
                  elevation: 5,
                  child: Padding(padding: EdgeInsets.symmetric(vertical: 10,horizontal: 15),
                      child: new Column(
                    children: <Widget>[
                      new ListTile(
                        leading: new Icon(FontAwesomeIcons.info,color: Colors.red[700],),
                        title: new Text("Previous Session Balance Fees",style: TextStyle(color: Colors.grey,fontSize: 14),),
                        contentPadding: EdgeInsets.all(0),
                      ),
                      new ListTile(
                        leading: new Icon(Icons.date_range,color: Colors.red[700],),
                        title: new Text.rich(
                          TextSpan(
                            children: <TextSpan>[
                              TextSpan(text: "Due Date:    " , style: TextStyle(color: Colors.red[700],fontWeight: FontWeight.bold,fontSize: 14)),
                              TextSpan(text: "20-09-1998", style: TextStyle(color: Colors.green,fontSize: 14)),
                            ],
                          ),),
                          contentPadding: EdgeInsets.all(0),
                      ),
  ...
  onSearchTextChanged(String text) async {
    ...
    _Feescarryforwarddetails.forEach((vehicleDetail) {
      if (vehicleDetail.studentname.toLowerCase().contains(text.toLowerCase())
  ...
  List<feescarryforwarddetails> _searchResult = [];
  List<feescarryforwarddetails> _Feescarryforwarddetails = [];

}

class feescarryforwarddetails {
  String id,studentname,admnno,admissiondate,balance;

  feescarryforwarddetails({this.balance,this.id,this.studentname,this.admissiondate,this.admnno});

  factory feescarryforwarddetails.fromJson(Map<String, dynamic> json) {
    return new feescarryforwarddetails(
        balance: json['name'].toString() ,
        id:  json['student_id'].toString() ,
        admissiondate:  json['name'].toString() ,
