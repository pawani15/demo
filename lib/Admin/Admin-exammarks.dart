...
class AdminExamMarksPage extends StatefulWidget {

  @override
  State<StatefulWidget> createState() => new _AdminExamMarksPageState();
}

class _AdminExamMarksPageState extends State<AdminExamMarksPage> {
  ...
  Future<Null> Showreport() async {
    ...
    var url = await Constants().Clienturl() + Constants.LoadExamamrks_Admin;
    Markslist.clear();
    ...
    http
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          setState(() {
            if(responseJson['result'].containsKey("substudent_list")){
              Markslist = responseJson['result']['substudent_list'];
            for(int i=0;i<Markslist.length;i++){
              Markslist[i]['marks'] = new TextEditingController(text: Markslist[i]['mark_obtained']);
              Markslist[i]['comment'] = new TextEditingController(text: Markslist[i]['comment']);
              }
            }
            else
              Markslist = new List();
  ...
  UpdateMarks() async{
    ...
    body['section_id'] = sectionamp[section];
    body['subject_id'] = subjectsmap[subject];
    body['total_subject_marks'] = totalmarks.text;
    Map body1 = new Map();
    for(int i=0;i<Markslist.length;i++){
      body1['marks_obtained_${Markslist[i]['mark_id']}'] = Markslist[i]['marks'].text;
      body1['comment_${Markslist[i]['mark_id']}'] = Markslist[i]['comment'].text;
    }
    body['studnet_mark_data'] = json.encode(body1);

    var url = await Constants().Clienturl() + Constants.UpdateExamamrks_Admin;
    print("url--"+url+'body is${json.encode(body)} $body');
    http.post(url,headers: {"Content-Type": "application/x-www-form-urlencoded"}, body: body)
        .then((response) {
      if (response.statusCode == 200) {
                                          ...
                                          children: <Widget>[
                                            ...
                                            new Expanded(
                                              child: new Padding(
                                                  ...
                                                  EdgeInsets.all(5),
                                                  child: new Text(
                                                    (Markslist[index]['student_first_name']).toString() + " "+Markslist[index]['student_last_name'].toString(),
                                                    textAlign: TextAlign.center,
                                                    style: TextStyle(
                                                        color: Colors.black,
                                                        fontSize: 11),
