...
class HomeworklistPage extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => new _HomeworklistPageState();
}
...
class _HomeworklistPageState extends State<HomeworklistPage> {
  ...
  Future<Null> LoadHomework(String load) async {
    ...
    var url = '';
    url = await Constants().Clienturl() + Constants.LoadHomework_Admin;

    Map<String, String> body = new Map();
    if(load == "load") {
      body['class_id'] = classname == "" ? "" : classmap[classname];
      body['subject_id'] = subjectname == "" ? "" : subjectmap[subjectname];
      body['assign_date'] =
  ...
  _navigatetoclasses1(BuildContext context) async {
    final result =  await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => Search(title: "Classes",duplicateitems: classlist,)),
    );
    ...
    FocusScope.of(context).requestFocus(FocusNode());
    if(result != null) {
      LoadSubjects(classmap[classname]);
    }
  }
  ...
  _navigatetosubjects1(BuildContext context) async {
    String result = await Constants().Selectiondialog(context, "Subjects", subjectsslist);
    setState(() {
  ...
  Future<Null> CreateHomework(String type,Homeworkdetails homeworkdetails) async {
    ...
    Constants().onLoading(context);
    var url="";
    url = await Constants().Clienturl() + Constants.CRUDHomework_Admin;
  ...
  Delete(Homeworkdetails Homeworkdetails) async{
    ...
    var  url = await Constants().Clienturl() + Constants.CRUDHomework_Admin;
      ...
      body: new Stack(
        children: <Widget>[
          ...
          _loading ? new Constants().bodyProgress : new Container(
            margin: new EdgeInsets.only(top: 40,right: 5,left: 5),
            child: new ListView(
              children: <Widget>[
                new Stack(
                  children: <Widget>[
  ...
  onSearchTextChanged(String text) async {
    ...
    _Homeworkdetails.forEach((vehicleDetail) {
      if (vehicleDetail.subject.toLowerCase().contains(text.toLowerCase()) || vehicleDetail.classname.toLowerCase().contains(text.toLowerCase())
          || vehicleDetail.duedate.toLowerCase().contains(text.toLowerCase())|| vehicleDetail.assigndate.toLowerCase().contains(text.toLowerCase())
          || vehicleDetail.created_by.toLowerCase().contains(text.toLowerCase()) || vehicleDetail.homework.toLowerCase().contains(text.toLowerCase()))
...
class _AddlasssPagePageState extends State<Assigntostudents> with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    classname = widget.details.classname;
    LoadSections(widget.classmap[widget.details.classname]);
  }

  Future<Null> AssignHomework() async {
    Constants().onLoading(context);
    var url="";
      url = await Constants().Clienturl() + Constants.CRUDAssignedhomeworks_Admin;
    String empid = await sp.ReadString("Userid");

    Map body = new Map();
    body['login_user_id'] = empid;
    if(assignid != '' || assignid != null)
      body['assgn_id'] = assignid;
  ...
  Future<Null> LoadAssignedhomework() async {
    String empid = await sp.ReadString("Userid");
    var url = await Constants().Clienturl() + Constants.LoadAssignedhomework_Admin;
    Map<String, String> body = new Map();
    body['hw_id'] = widget.details.id;
    body['cls_id'] = widget.classmap[classname];
    body['section_id'] = sectionamp[section];
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['status'].toString() == "true") {
          ...
          try {
            ...
            assignid = responseJson['result']['assgn_id'].toString();
            setState(() {
              for(int i=0;i<assignedlist.length;i++){
                for(int j=0;j<studentlist.length;j++){
                  print("stuid--"+studentlist[j]['student_id']+"--"+assignedlist[i].toString().split("_")[1]);
                  if(assignedlist[i].toString().split("_")[1] == studentlist[j]['student_id'].toString())
                    studentlist[j]['check'] = true;
                }
              }
            });
          }catch(e){
  ...
  Widget build(BuildContext context) {
    // TODO: implement build
    return _loading ? new Constants().bodyProgress :  new Column(
      children: <Widget>[
        new Container(margin: new EdgeInsets.all(5.0),
            child : new Row(crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisAlignment: MainAxisAlignment.center,
                      ...
                      child: new Container(
                          ...
                          child: new Row(
                            ...
                            children: <Widget>[
                              ...
                              new Padding(padding: EdgeInsets.only(left: 5.0),child: Text("Close",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 11),),)
                            ],
                          )))),flex: 1,),
              new Expanded(child:new Container(
                  margin: new EdgeInsets.all(0.0),
...
class Homeworkdetails {
  String description, homework, classname,subject,id,assigndate,duedate,docwork,created_by;

  Homeworkdetails({this.description, this.homework, this.classname,this.subject,this.id,this.assigndate,this.duedate,this.docwork,this.created_by});
